let PhoneNum: number[] = [6, 5, 3, 2, 8, 4, 7, 3, 8, 0];

function CreatePhoneNum(PhoneNum : number[]): void {
...
let result: number[] = numsB.concat(numsA);
console.log(result);

//

let nums1 : number[] = [1, 2, 3];
let nums2 : number[] = [4, 8, 9];
let array: number[] = nums1.concat(nums2);
let middleArray: number[] = [];
console.log(array);
